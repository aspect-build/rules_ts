...
bazel build :ts && exit_with_message "Case 1: expected ts worker to report errors for evil.ts but it didn't"

message="evil.ts(1,1): error TS2304: Cannot find name 'evilcode'."
bazel build :ts 2>&1 | grep "$message" || exit_with_message "Case 1: expected worker to report \"$message\" but it didn't" 
cleanup
