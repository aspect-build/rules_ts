...
#
# Usage:
#   ts_project_options_validator.sh <jq> <tsc> <wrapper_bin_path> <extends> <resolved_out> <target_label> <tsconfig_dir> <attr>=<value>...
#
# <resolved_out> (a declared output) receives the `tsc --showConfig` output:
# the resolved tsconfig this action validates, and the validation output.
#
...
# jq emits CRLF line endings on Windows.
cfg="${cfg//$'\r'/}"
