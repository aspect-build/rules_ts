...
GEN_ES_PLUGIN_TOOLCHAIN = "@aspect_rules_ts//ts/toolchains:ts_protoc_plugin"
...
def _ts_proto_aspect_impl(target, ctx):
    ...
    for src in proto_srcs:
        ...
        ctx.actions.run_shell(
            command = "{protoc} --plugin=protoc-gen-es={gen_es} --es_out={bindir} {args} {sources}".format(
                protoc = proto_info.proto_compiler.executable.path,
                sources = " ".join([p.path for p in proto_srcs]),
                gen_es = gen_es_info.plugin.executable.path,
                bindir = ctx.bin_dir.path,
                args = gen_es_info.out_replacement_format_flag,
            ),
            env = {"BAZEL_BINDIR": ctx.bin_dir.path},
            tools = [gen_es_info.plugin, proto_info.proto_compiler],
    ...
    return [
        js_info(
            ...
            transitive_sources = js_lib_helpers.gather_transitive_sources(js_outputs, ctx.rule.attr.deps),
            transitive_types = js_lib_helpers.gather_transitive_types(dts_outputs, ctx.rule.attr.deps),
            npm_sources = js_lib_helpers.gather_npm_sources(srcs = [], deps = deps),
            npm_package_store_infos = js_lib_helpers.gather_npm_package_store_infos(deps),
        ),
        DefaultInfo(
            #  files = depset(js_outputs + dts_outputs),
        ),
    ]

ts_proto_aspect = aspect(
    implementation = _ts_proto_aspect_impl,
