...
if (Array.isArray(ts.ignoredPaths)) {
    ts.ignoredPaths = ts.ignoredPaths.filter(
        (ignoredPath) => ignoredPath != '/node_modules/.'
    )
}
        ...
        const walk = (node, prefix) => {
            ...
            for (const [index, key] of subnodes.entries()) {
                ...
                if (subnode[Type] == TYPE.SYMLINK) {
                    output.push(
                        `${prefix}${parts[0]}${key} -> ${subnode[Symlink]}`
                    )
    ...
    function add(p) {
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            if (node && node[Type] == TYPE.SYMLINK) {
                ...
                debug(
                    `WEIRD_BAZEL_6_BEHAVIOR: stopping at ${parents.join(
                        path.sep
                    )}`
                )
                return
            ...
            if (typeof node[segment] != 'object') {
                const possiblyResolvedSymlinkPath =
                    followSymlinkUsingRealFs(currentp)
        ...
        for (const segment of segments) {
            ...
            if (node[Type] == TYPE.SYMLINK) {
                ...
                if (newSymlinkPath == currentp) {
                    ...
                    debug(
                        `${currentp} is no longer a symlink since ${currentp} == ${newSymlinkPath}`
                    )
                    node[Type] = TYPE.FILE
                    delete node[Symlink]
                } else if (node[Symlink] != newSymlinkPath) {
                    debug(
                        `updating symlink ${currentp} from ${node[Symlink]} to ${newSymlinkPath}`
                    )
...
function createEmitAndLibCacheAndDiagnosticsProgram(
    ...
    const outputSourceMapping = (host.outputSourceMapping =
        host.outputSourceMapping || new Map())
    ...
    builder.emit = (
        ...
        if (!targetSourceFile) {
            for (const [path, entry] of outputCache.entries()) {
                ...
                if (
                    sourcePath != NOT_FROM_SOURCE &&
                    !builder.getSourceFile(sourcePath)
                ) {
                    debug(
                        `createEmitAndLibCacheAndDiagnosticsProgram: deleting ${sourcePath} as it's no longer a src.`
                    )
    ...
    host.getSourceFile = (fileName) => {
        ...
        if (
            sf &&
            path
                .relative(host.getDefaultLibLocation(), fileName)
                .startsWith('..')
        ) {
            debug(
                `createEmitAndLibCacheAndDiagnosticsProgram: putting default lib ${fileName} into cache.`
            )
...
function createProgram(args, inputs, output, exit) {
    ...
    const tsconfig = path.relative(
        execroot,
        path.resolve(bin, cmd.options.project)
    ) // bazel-bin/<cfg>/bin/<pkg>/<options.project>
    const cfg = path.relative(execroot, bin) // /bazel-bin/<cfg>/bin
    const executingFilePath = path.relative(
        execroot,
        require.resolve('typescript')
    ) // /bazel-bin/<opt-cfg>/bin/node_modules/tsc/...
    ...
    function invalidate(filePath, kind) {
        ...
        if (
            ...
            kind === ts.FileWatcherEventKind.Created
        ) {
    ...
    function enableStatisticsAndTracing() {
        if (
            ...
            host.optionsToExtend.diagnostics ||
            host.optionsToExtend.extendedDiagnostics
        ) {
        ...
        if (
            (compilerOptions.generateTrace ||
                host.optionsToExtend.generateTrace) &&
            ts.startTracing &&
            !ts.tracing
        ) {
            ts.startTracing(
    ...
    function updateOutputs() {
        outputs.clear()
        if (
            host.optionsToExtend.tsBuildInfoFile ||
    ...
    function readCompilerOptions() {
        ...
        const parsedCommandLine = ts.parseJsonConfigFileContent(
            raw.config,
            sys,
            path.dirname(cmd.options.project)
        )
    ...
    function readFile(filePath, encoding) {
        ...
        if (
            ...
            path
                .relative('/' + executingDirectoryPath, filePath)
                .startsWith('..') &&
            !outputs.has(filePath)
        ) {
            output.write(
                `tsc tried to read file (${filePath}) that wasn't an input to it.` +
                    '\n'
            )
            throw new Error(
                `tsc tried to read file (${filePath}) that wasn't an input to it.`
...
async function emit(request) {
    ...
    if (request.arguments.includes('--bazelValidationFile')) {
        const [, validationPath] = request.arguments.splice(
            request.arguments.indexOf('--bazelValidationFile'),
            2
        )
    ...
    const diagnostics = ts
        .getPreEmitDiagnostics(program, undefined, cancellationToken)
...
if (
    require.main === module &&
    worker_protocol.isPersistentWorker(process.argv)
) {
    ...
    if (args.includes('--bazelValidationFile')) {
        const [, validationPath] = args.splice(
            args.indexOf('--bazelValidationFile'),
            2
        )
        fs.writeFileSync(path.resolve(process.cwd(), validationPath), '')
    }
...
module.exports.__do_not_use_test_only__ = {
    createFilesystemTree: createFilesystemTree,
    emit: emit,
    workers: workers,
}
