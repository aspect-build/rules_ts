...
test('readDirectory', () => {
    ...
    create_dir_symlink(
        tree,
        'bazel-out/cfg/bin/node_modules/pkg',
        'bazel-out/cfg/bin/node_modules/.store/pkg'
    )

    assert.deepStrictEqual(tree.readDirectory('bazel-out'), ['bazel-out/cfg'])
    assert.deepStrictEqual(tree.readDirectory('bazel-out/cfg'), [
        'bazel-out/cfg/bin',
    ])
    assert.deepStrictEqual(tree.readDirectory('bazel-out/cfg/bin'), [
        'bazel-out/cfg/bin/input.js',
        'bazel-out/cfg/bin/node_modules',
    ])
    assert.deepStrictEqual(
        tree.readDirectory('bazel-out/cfg/bin/node_modules/pkg'),
        ['bazel-out/cfg/bin/node_modules/pkg/index.js']
    )
})
...
test('getDirectories', () => {
    ...
    assert.deepStrictEqual(
        tree.getDirectories('bazel-out/cfg/bin/node_modules'),
        ['.store', 'pkg']
    )
})
...
test('realpath', () => {
    ...
    assert.equal(
        tree.realpath('bazel-out/cfg/bin/node_modules/pkg'),
        '/bazel-out/cfg/bin/node_modules/.store/pkg'
    )
})
...
test('add', () => {
    ...
    assert.deepStrictEqual(tree.readDirectory('tree'), [
        'tree/subtree',
        'tree/input.js',
    ])
...
test('remove', () => {
    ...
    assert.ok(!tree.fileExists('bazel-out/cfg/bin/input.js'))
    assert.deepEqual(tree.readDirectory('bazel-out/cfg/bin'), [
        'bazel-out/cfg/bin/node_modules',
    ])
    ...
    assert.ok(
        !tree.directoryExists('bazel-out/cfg/bin/node_modules/.store/pkg')
    )
...
test('dangling symlink', () => {
    ...
    fs.symlinkSync(
        path.join(root, 'dangle'),
        path.join(root, 'may_dangle'),
        'dir'
    )
    tree.add('may_dangle')
...
test('multiple file watchers get notified', () => {
    ...
    assert.deepStrictEqual(callback1.calls, [{ arguments: ['inputs/1.js', 0] }])
    assert.deepStrictEqual(callback2.calls, [
        { arguments: ['inputs/1.js', 0] },
        { arguments: ['inputs/1.js', 2] },
    ])
    assert.deepStrictEqual(callback3.calls, [{ arguments: ['inputs/1.js', 0] }])
    assert.deepStrictEqual(callback4.calls, [
        { arguments: ['inputs/1.js', 0] },
        { arguments: ['inputs/1.js', 2] },
    ])
})
...
test('recursive dir watchers get notified', () => {
    ...
    assert.deepEqual(
        callback.calls.map((call) => call.arguments[0]),
        [
            'recursive/1.js',
            'recursive/to',
            'recursive/to/1.js',
...
test('no double calls for recursive dir watchers', () => {
    ...
    assert.deepStrictEqual(callback.calls, [
        { arguments: ['recursive_behavior/1.js', 0] },
    ])
})
...
test('mix of recursive and non-recursive should work', () => {
    ...
    create_file(tree, 'recursive/1.js')
    assert.deepStrictEqual(callback.calls, [
        { arguments: ['recursive/1.js', 0] },
    ])
    assert.deepStrictEqual(recursiveCallback.calls, [
        { arguments: ['recursive/1.js', 0] },
    ])
})
