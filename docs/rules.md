...
| <a id="ts_project-tsc"></a>tsc |  Label of the TypeScript compiler binary to run. This allows you to use a custom API-compatible compiler in place of the regular `tsc` such as a custom `js_binary` or Angular's `ngc`. compatible with it such as Angular's `ngc`.<br><br>See examples of use in [examples/custom_compiler](https://github.com/aspect-build/rules_ts/blob/main/examples/custom_compiler/BUILD.bazel)   |  `Label("@npm_typescript//:tsc")` |
| <a id="ts_project-tsc_worker"></a>tsc_worker |  Label of a custom TypeScript compiler binary which understands Bazel's persistent worker protocol.   |  `Label("@npm_typescript//:tsc_worker")` |
...
| <a id="ts_project-validator"></a>validator |  Label of the tsconfig validator to run when `validate = True`.   |  `Label("@npm_typescript//:validator")` |
